	...
	<view class="gpp-datePicker">
		...
		<view class="g-dp-content" :class="{'show':pipkerShowFlag}" @touchmove.stop.prevent catchtouchmove="true">
			<view class="g-dp-ctt-head">
				<view class="g-dp-ctt-hd-btn" :style="{'color':themeColor}" @click.stop="onCancel">取消</view>
				<text>{{title}}</text>
			...
			<view class="g-dp-ctt-wrapper">
				<picker-view :indicator-style="indicatorStyle" :value="selectedValue" @change="wrapperChange">
					<picker-view-column v-if="showFormDate!=3">
						<view class="g-dp-ctt-wp-item" :style="yearPickerIndex===index?'color:'+themeColor:''" v-for="(item,index) in years" :key="index">{{item}}年</view>
					</picker-view-column>
					<picker-view-column v-show="showFormDate==0||showFormDate==1||showFormDate==3">
						<view class="g-dp-ctt-wp-item" :style="monthPickerIndex===index?'color:'+themeColor:''" v-for="(item,index) in months" :key="index">
							{{dateFormate(item)}}月
						</view>
					</picker-view-column>
					<picker-view-column  v-show="showFormDate==0">
						<view class="g-dp-ctt-wp-item" :style="dayPickerIndex===index?'color:'+themeColor:''" v-for="(item,index) in days" :key="index">{{dateFormate(item)}}日
						</view>
					</picker-view-column>
				</picker-view>
			</view>
...
<script>
	...
	export default {
		props: {
			...
			showFormDate:{				type:Number,				default(){					return 0; 
			// 0显示年月日 1 显示年月 2显示年 3显示月				
			}			},
		},
		data() {
			return {
		...
		methods: {
			...
			/* 	this.yearPickerIndex=detailValue[0];
				this.monthPickerIndex=detailValue[1];
				this.dayPickerIndex=detailValue[2]; */
			},
			/**
			 * props值校验
			 */
			...
			wrapperChange(e) {
				...
				this.monthPickerIndex=detailValue[1];
				this.dayPickerIndex=detailValue[2];
				let selectedDate = this.years[detailValue[0]] + "-" + this.months[detailValue[1]] + "-" + this.days[
					detailValue[2]];
				if (this.selectedValue[0] != detailValue[0]) {
					this.getMonths(selectedDate);
					selectedDate = this.years[detailValue[0]] + "-" + this.months[detailValue[1]] + "-" + this.days[
						detailValue[2]];
			...
			getDateValue(pikerValue) {
				return this.years[pikerValue[0]] + "-" + this.dateFormate(this.months[pikerValue[1]]) + "-" + this
					.dateFormate(this.days[pikerValue[2]]);
			}
		}
	}
</script>
